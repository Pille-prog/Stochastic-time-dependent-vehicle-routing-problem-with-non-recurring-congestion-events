...
def lookup(**speeds_by_minute: float) -> dict:
    """Endpoint observations, e.g. ``lookup(m420=0.6, m540=0.9)``."""
    return {(LINK, int(name[1:])): speed for name, speed in speeds_by_minute.items()}
...
class TestOutsideTheWindows:
    ...
    def test_window_boundaries_are_exclusive(self, minute):
        ...
        endpoints = lookup(m420=0.6, m540=0.9, m660=0.6, m840=0.9, m960=0.6, m1080=0.9)
...
class TestInsideTheWindows:
    def test_morning_window_midpoint_is_the_average_of_the_endpoints(self):
        assert _interpolated_speed(row(480), lookup(m420=0.6, m540=0.9)) == pytest.approx(0.75)

    def test_first_minute_inside_the_morning_window(self):
        expected = 0.6 * (119 / 120) + 0.9 * (1 / 120)
        assert _interpolated_speed(row(421), lookup(m420=0.6, m540=0.9)) == pytest.approx(expected)

    def test_last_minute_inside_the_morning_window(self):
        expected = 0.6 * (1 / 120) + 0.9 * (119 / 120)
        assert _interpolated_speed(row(539), lookup(m420=0.6, m540=0.9)) == pytest.approx(expected)

    def test_midday_window_blends_between_660_and_840(self):
        expected = 0.8 * (2 / 3) + 0.2 * (1 / 3)  # minute 720: 60 of 180 minutes in
        assert _interpolated_speed(row(720), lookup(m660=0.8, m840=0.2)) == pytest.approx(expected)

    def test_evening_window_blends_between_960_and_1080(self):
        assert _interpolated_speed(row(1020), lookup(m960=0.4, m1080=0.6)) == pytest.approx(0.5)

    def test_original_speed_inside_a_window_is_ignored(self):
        interpolated = _interpolated_speed(row(480, speed=99.0), lookup(m420=0.6, m540=0.9))
        assert interpolated == pytest.approx(0.75)


class TestMissingObservations:
    def test_missing_left_endpoint_yields_none(self):
        assert _interpolated_speed(row(480), lookup(m540=0.9)) is None

    def test_missing_right_endpoint_yields_none(self):
        assert _interpolated_speed(row(480), lookup(m420=0.6)) is None
