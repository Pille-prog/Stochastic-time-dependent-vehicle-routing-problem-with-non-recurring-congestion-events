...
fixtures). The legacy ``ClientGenerator`` hardcodes a 1,900-node universe and a
60-client floor, so demand comes from the ported ``ClientGenerator`` with
fixture-sized parameters for BOTH sides — ticket 06 proved its stream consumption
is bit-identical to the legacy's, so the episodes that follow start from identical
RNG state. Each side re-seeds before running, and both must leave both global
streams at the same position afterwards.
"""
