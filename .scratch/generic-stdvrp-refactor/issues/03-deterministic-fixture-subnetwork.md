...
- [x] Fixture (links subset + matching morning/afternoon speed subsets) committed under the test fixtures directory, total well under 1 MB (770 KB: 45 nodes, 116 arcs, day 601, periods from 08:00 onward with no upper cut)
...
5. Fixture `all_shortest_paths.csv` is recomputed on the sub-network (day-601 mean-speed travel times), guaranteeing path arcs exist in the mini network; duplicate-arc semantics follow legacy `read_network` (last row wins). **Caution for 05/06**: its AverageTime/Length values are self-consistent but do NOT reproduce the legacy file's 44-day aggregated semantics — never use them as expected values against legacy outputs.
6. Post-review fixes (same day): removed the upper period cut (legacy has none; interpolation and trip completion read past minute 780 — fixture grew to 770 KB), corrected the `data_average` semantics claim in README/docstring, amended spec.md Phase 0 acceptance to match the ADR addendum. Known accepted debt: `period_start_minute` duplicated between script and test (third copy expected in ticket 05 — unify then); format compatibility is asserted structurally, not by feeding a legacy reader (ticket 04 covers the real run).
